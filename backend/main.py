...
- 패커 탐지 결과: {data.get('packer', 'N/A')}

(5) YARA 룰 매칭
- 탐지된 룰 수: {len(data.get('yara_rules', [])) if isinstance(data.get('yara_rules'), list) else 1}
- 커버리지 및 설명: 추후 반영 필요
""",
        3: f"""
③ 동적 분석
...
- 제공된 각 기술에 대한 설명을 예를 들어
  - [T1082] "System Information Discovery"는 시스템 정보를 수집하는 기술로, 공격자가 시스템의 구성 요소를 이해하고 취약점을 찾는 데 사용됨.
  - [T1134] "Access Token Manipulation"은 사용자의 액세스 토큰을 조작하여 권한 상승을 시도하는 기술임.
  - [T1547.001] "Boot or Logon Autostart Execution"은 시스템 부팅 또는 로그인 시 자동 실행되도록 설정하여 지속성을 유지하는 기술임.
- 마지막으로, 각 기술들을 종합해 동작 과정을 **자세히** 설명하며 마무리.
- 보고서 형식으로 작성
""",
        6: f"""
⑥ CWE 매핑
- 매핑된 CWE ID를 각 CWE에 대한 설명과 1000.csv 매핑 기준을 설명:  
{format_cwe(data['CWE'])}
- 제공된 각 기술에 대한 설명을 예를 들어
  - [CWE-785] "use of path manipulation function without maximum-sized buffer"는 경로 조작 함수 사용 시 출력 버퍼 크기를 충분히 지정하지 않아 버퍼 오버플로우 등의 취약점으로 이어질 수 있는 경우에 해당함.
  - [CWE-119] "Improper Restriction of Operations within the Bounds of a Memory Buffer"는 메모리 범위를 벗어난 쓰기가 발생하여 메모리 손상으로 이어질 수 있는 경우에 해당함.
- 마지막으로, 각 기술들을 종합해 동작 과정을 **자세히** 설명하며 마무리.
- 보고서 형식으로 작성
"""
    }
    ...
    if not prompt_body or not section_title:
...
출력은 반드시 마크다운 없이 일반 텍스트만 사용해주세요.
정확하고 구체적인 문장으로 풀어써주세요. 문체를 “~함” 으로 통일해주세요.

<분석 대상 개요>
- 파일명: {meta.get("module","")}
- 해시(SHA-256): {meta.get("sha256","")}
- 형식: {meta.get("fileType","")}
- 크기: {meta.get("fileSize","")} bytes
