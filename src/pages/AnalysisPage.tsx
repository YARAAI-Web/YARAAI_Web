...
interface AnalysisResult {
  ...
  h_code: string[]
  virustotal: {
    hashes: { md5: string; sha1?: string; sha256: string; vhash?: string }
    file_type: string
    magic: string
    file_size: number
    trid?: { file_type: string; probability: number }[]
    analysis: { detectiteasy?: { result: string }; magika?: { result: string } }
    packer?: string
  }
  yara_rule: string
...
<head>
  ...
  <style>
    ...
    pre { white-space:pre-wrap; font-family:'Segoe UI','Malgun Gothic'; font-weight:350; line-height:1.6; margin-top:8px; }
  ...
  ${SECTIONS.map((section, idx) => {
    if (idx === 3) {
      ...
      <div class="iframe-container">
        <iframe src="${window.location.origin}/static/callgraphs/${baseName}.html" style="width:100%;height:100%;border:none;" sandbox="allow-scripts allow-same-origin"></iframe>
      </div>`
    } else if (idx === 0) {
      return `<h2>${section}</h2>
      <table class="vt-table"><tbody>
        <tr><th>MD5</th><td>${data.virustotal.hashes.md5}</td></tr>
        <tr><th>SHA-1</th><td>${data.virustotal.hashes.sha1 ?? '—'}</td></tr>
        <tr><th>SHA-256</th><td>${data.virustotal.hashes.sha256}</td></tr>
        <tr><th>Vhash</th><td>${data.virustotal.hashes.vhash ?? '—'}</td></tr>
        <tr><th>File type</th><td>${data.virustotal.file_type}</td></tr>
        <tr><th>Magic</th><td>${data.virustotal.magic}</td></tr>
        <tr><th>File size</th><td>${data.virustotal.file_size.toLocaleString()} bytes</td></tr>
        ${
          data.virustotal.trid
            ? `<tr><th>TrID 상위 3개</th><td>${data.virustotal.trid
                .slice(0, 3)
                .map((t) => `${t.file_type} (${t.probability}%)`)
                .join(', ')}</td></tr>`
            : ''
        }
        <tr><th>DetectItEasy</th><td>${
          data.virustotal.analysis.detectiteasy?.result ?? '—'
        }</td></tr>
        <tr><th>Magika</th><td>${
          data.virustotal.analysis.magika?.result ?? '—'
        }</td></tr>
        <tr><th>Packer</th><td>${data.virustotal.packer ?? '—'}</td></tr>
      </tbody></table>
      <pre>${allTexts[0].split('<VirusTotal')[0].trim()}</pre>`
    } else {
      return `<h2>${section}</h2><pre>${allTexts[idx]}</pre>`
    }
  }).join('')}
</body>
</html>`
    saveAs(new Blob([html], { type: 'text/html' }), `${baseName}.html`)
  }

  const downloadSuricataJSON = () => {
    const a = document.createElement('a')
    a.href = `/meta_json/${baseName}_suricata.json`
    ...
    <>
      ...
      <div className="flex flex-col min-h-screen bg-white">
        ...
        <div className="px-8 pt-4 pb-0">
          <div className="max-w-5xl mx-auto relative flex bg-white p-2 gap-6">
            <div className="w-[200px] bg-orange-500 text-white font-bold text-center p-2 rounded-md text-lg">
              Likely Malicious
            </div>
            ...
            <div className="absolute -top-[20px] right-[30px] flex flex-col space-y-[2px]">
              <button
                ...
                className="bg-[#1b65fe] hover:bg-[#F2F2F7] text-white font-bold text-xl px-[20px] py-[7px] rounded-full border-[0.1px] border-[rgba(0,0,0,0.15)]"
              >
                Report
              </button>
              <button
